...
# %% [markdown]
# # Brown & Minion 1995 — double shear layer, wavelet AMR
#
# [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/slitvinov/wavegp-public/blob/main/brown_wavelet.ipynb)
#
# Self-contained notebook that reproduces the **double shear layer** of
...
#
# The solver `main.c` (CUP2D `brown/` @ commit `1331f840`, "use wavelet
# adaptation") carries a **wavelet / multiresolution refinement indicator**:
# each block is restricted 2×2→1, prolongated back, and the interpolation
# error — the wavelet *detail* coefficient — drives refinement.  Blocks where
# the detail exceeds `-Rtol` are refined; the mesh tracks the shear layers.
#
# It runs one adaptive case (`levelStart 2 → levelMax 5`, a graded 64-256
# mesh) and lays the computed vorticity iso-lines + the adaptive mesh
# side-by-side with the paper's 256² panels.
#
# `main.c` and the scripts are embedded below (`%%writefile`); only the paper
# reference panels are downloaded.
...
# iso-line extraction library `make_amr_panels.py` uses.
...
# ## 2. Solver source
#
# The next cells write the embedded source files to disk.
...
# ## 3. Reference figures
#
# Download the paper's 256² panels (Figs. 2-3, label C).

# %%
RAW = "https://raw.githubusercontent.com/slitvinov/wavegp-public/main"
os.makedirs("ref", exist_ok=True)
for fig in ("fig2_C", "fig3_C"):
    rel = f"ref/{fig}.png"
    subprocess.run(["curl", "-sSL", "-o", rel, f"{RAW}/{rel}"], check=True)
    print("fetched", rel)

# %% [markdown]
# ## 4. Generate the lifted-wavelet stencil tables
#
# `gen_table.py` writes the `tab_*.bin` files used for the cross-level
...
# %%
subprocess.run(["gcc", "-O2", "-o", "main", "main.c", "-lm"], check=True)
print("built ./main")

# %% [markdown]
# ## 6. Adaptive run
#
# `-AdaptSteps 2` re-adapts every 2 steps; the wavelet indicator refines from
# `levelStart 2` toward `levelMax 5`.  `-Rtol 0.1` gives a graded 64-256 mesh
# (commit `1331f840`'s own `run.sh` used `-Rtol 0.005`, which over-refines the
# whole domain to the finest level).  Runs to t = 1.2 — dump 2 is t=0.8
# (Fig. 2), dump 3 is t=1.2 (Fig. 3).  Takes a few minutes.

# %%
subprocess.run(
    ["./main", "-AdaptSteps", "2", "-CFL", "0.8", "-Rtol", "0.1",
     "-levelMax", "5", "-levelStart", "2", "-nu", "1e-4",
     "-sdump", "0", "-tdump", "0.4", "-tend", "1.2"], check=True)
print("run done")

# %% [markdown]
# ## 7. Comparison: paper vs adaptive run
#
# `make_amr_panels.py` extracts the vorticity iso-lines from the AMR cells
# with `amriso` and writes two figures:
# `comparison_amr.png` (paper 256² vs AMR, with the adaptive mesh) and
# `panel_amr.png` (AMR vorticity and mesh at t=0.8 and t=1.2).

# %%
subprocess.run([sys.executable, "make_amr_panels.py"], check=True)

# %%
from IPython.display import Image, display
display(Image("comparison_amr.png"))
display(Image("panel_amr.png"))
